...
for(const line of data.split('\n')) {
    ...
    if(doestFullInclude(start1, end1, start2, end2) || doestFullInclude(start2, end2, start1, end1)) {
        result ++;
    }
}
...
function doestFullInclude(aStart: number, aEnd: number, bStart: number, bEnd: number) {
